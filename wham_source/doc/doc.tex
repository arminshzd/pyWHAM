...
Small change to Makefile to correctly clean out the nr/ directory, suggested by
Lorenzo Rovigatti.

\subsection{New in release 2.0.11}

Two small changes: I added a quick compile-time option to use reduced energy units (suggested by Trung Dac Nguyen), and I fixed a compile problem on gcc 10 and later.

\subsection{New in release 2.0.10.1}
...
\subsection{New in release 2.0.10}
...
\subsection{New in release 2.0.9}
...
\subsection{New in release 2.0.8}
...
\subsection{New in release 2.0.7}
...
\subsection{New in release 2.0.6}
...
\subsection{New in release 2.0.4 and 2.0.5}
...
As of version 2.1.0, we've switched to using Cmake for the build, which should
be simpler than the previous system. To build the code, you should:
...
This will compile wham and wham-2d, with the intermediate files stored in the
CMakeFiles directory. You can use the two binaries directly, or install them
using:
...
This will install them in /usr/local/bin. If you want to install them (plus the
docs) elsewhere, you would run
...
\begin{verbatim}
wham [units <real|metal|lj|...>] [P|Ppi|Pval] hist_min hist_max num_bins \ 
     tol temperature numpad metadatafile freefile [num_MC_trials randSeed]
\end{verbatim}

The first optional argument specifies the units used for the reaction
coordinate. The default is kcal/mol, as before, but now there are other options
as well, adopted from the units used in LAMMPS:

\begin{itemize}
    \item real: kcal/mol-Kelvin
    \item lj: sets $k_B = 1.0$, so really a reduced unit
    \item metal: eV/Kelvin
    \item si: Joules/Kelvin 
    \item cgs: ergs/Kelvin
    \item electron: Hartrees/Kelvin
    \item micro: picogram-micrometer$^2$/microsecond$^2$-Kelvin
    \item nano: attogram-nanometer$^2$/nanosecond$^2$-Kelvin
\end{itemize}

The next (optional) argument specifies the periodicity of the reaction
...
specifies periodicity of some arbitrary amount, val, which should be an
...
particles are equally likely, giving a probability density (and thus a PMF) that
...
coordinate, so it produces free energy curves, not PMFs. As far as I can tell, this
